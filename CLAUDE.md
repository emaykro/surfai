...
| `PREDICTED_LEAD_THRESHOLD` | `0.7` | Min calibrated `model_prediction_score` for `score-predicted-leads.js` to generate a `lead_predicted` conversion. Prod runs `0.3` since 2026-07-22 — the v5 score distribution is bimodal and 0.5–0.7 adds almost nothing over real leads; 0.3 yields ~7 predicted/day of which ~2.5 are net-new non-converted signal. |
