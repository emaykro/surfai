...
## Post-hoc добавление: «где форварды?» диагностика

Вечером после деплоя контактного форвардера случился поучительный мини-инцидент. Тестер Ani M написала в @Surfaiask_bot, получила автоответ. Пользователь спросил: «я как админ никаких сообщений не получил, это же не норм?». Прошла долгая диагностика: проверили таймер, state-file, getUpdates queue, отправили три контрольных сообщения в @SurfaiOps_bot с идентичным форварду форматом — все ушли с `ok: true`. В итоге оказалось, что форварды **на самом деле приходили**, но пользователь их не видел, потому что **в @SurfaiOps_bot в одном чате смешаны два типа сигналов**: health-алерты от системного мониторинга и контактные форварды от лидов. Среди 🚨/✅/🧪 тестовых сообщений за день поток 📩 форвардов легко потерялся.

**Вывод** (добавлен в `feedback_telegram_alert_channels.md`): **один чат для двух разных классов уведомлений — UX-ловушка**. Алерты инфраструктуры (невидимая рутина) и лиды из продаж (требуют внимания) должны быть либо в отдельных ботах/чатах, либо иметь принципиально разный визуальный вес (вынесенные в закреп, жирный префикс, отдельный звук). Сейчас оставили как есть — но если лидов пойдёт больше 1-2 в день, стоит завести отдельный «@SurfaiLeadsBot» или третий чат с @SurfaiOps_bot именно для лидов.
